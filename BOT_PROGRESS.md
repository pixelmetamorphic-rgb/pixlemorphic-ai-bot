...
## 2026-09-23 Natural Skin XL failed user visual test; three private replacements
User reports RealVisXL V4 Natural Skin XL output unusably poor; remove from active private menu, preserve old route for audit, and keep successful Realism XL pinned unchanged. Research found fal FLUX.2 Dev with explicit checker-disable field **requiring account authorization**, fal Fooocus Quality with explicit checker field, and Runware's existing 9B image route supporting a separate experimental checker-off request (provider moderation may persist). Added `flux2photo` (fal-ai/flux-2), `fooocusphoto` (fal-ai/fooocus), and `flux2klein9blf` (runware:400@2), all 1K admin-only and experimental. No changed production model or public credit. No provider-billed test run. Proposed nominal 1K credit placeholders (20/15/2 respectively) remain unvalidated. Existing Kie offers no verified account-authorized model-independent safety-off override; existing Replicate Pony models still need one-by-one user tests. See NEXT_CHAT_HANDOFF.md and provider docs for endpoint/schema caveats.
## 2026-09-23 cleanup
Ten obsolete admin-only experimental image aliases and their rates/backends removed. Retain LOCKED Realism XL and exactly two Replicate Pony research candidates. All normal image models/plans/credits preserved. Admin can trigger metadata-only live Runware search with `/runwaremodels pony|cyberrealistic|realistic` to retrieve exact AIRs without a billable generation. No user-visible candidate model added from unverified catalog results; no paid tests.
