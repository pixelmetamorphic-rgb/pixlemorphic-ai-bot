...
function activateEdit(src) {
  if (!src.includes("fal-ai/flux-pro/kontext")) {
    ...
    if (modelStart < 0 || modelEnd < 0) throw new Error("Could not locate legacy SHARK model block");
...
function buildTarget(text) {
  return text.replaceAll("__BT__", "`");
}

function upgradeUI(src) {
  const keyboardBlock = buildTarget(String.raw`/* =========================
   KEYBOARDS
========================= */
...
function videoKeyboard() {
  return {
    inline_keyboard: [
      ...
      [
        { text: "🎞️ FPS", callback_data: "v:fps" },
        { text: "🔊 AUDIO", callback_data: "v:audio" }
      ],
      [
...
`);
  ...
  const homeBlock = buildTarget(String.raw`async function showHome(chatId, userId) {
  const plan = await getPlan(userId);
  const credits = await getCredits(userId);
  return sendMessage(
    chatId,
    __BT__🚀 PIXLEMORPHIC AI\n\nPremium AI Creation Studio\n\n💳 Plan: ${plan.toUpperCase()}\n⚡ Credits: ${credits}\n\nChoose your studio:__BT__,
    { reply_markup: homeKeyboard() }
  );
}

`);
  src = replaceBetween(src, "async function showHome(", "async function showImageMenu(", homeBlock + "async function showImageMenu(");

  const imageMenuBlock = buildTarget(String.raw`async function showImageMenu(chatId, userId) {
  await setFlow(userId, { step: "choose_model" });
  return sendMessage(
    chatId,
    __BT__🖼️ IMAGE STUDIO\n\nSelect an engine.\n\n🟢 LIVE = available now\n🟡 SOON = integration scheduled__BT__,
    { reply_markup: imageKeyboard() }
  );
}

`);
  src = replaceBetween(src, "async function showImageMenu(", "async function showVideoMenu(", imageMenuBlock + "async function showVideoMenu(");

  const videoMenuBlock = buildTarget(String.raw`async function showVideoMenu(chatId, userId) {
  await clearFlow(userId);
  return sendMessage(
    chatId,
    __BT__🎬 VIDEO STUDIO\n\nChoose an upcoming engine and preview the controls below.\n\n🎚️ Quality: 480p • 580p • 720p • 1080p • 4K\n⏱️ Duration: 5s • 8s • 10s • 15s\n🎞️ Frame rate: 24 • 30 FPS\n📐 Ratio: 16:9 • 9:16 • 1:1 • 4:3 • 3:4 • 21:9\n🔊 Native audio: model dependent\n\n🚧 Video generation is disabled until provider integration + cost testing are complete. Preview controls do not consume credits.__BT__,
    { reply_markup: videoKeyboard() }
  );
}

`);
  src = replaceBetween(src, "async function showVideoMenu(", "async function cmdCredits(", videoMenuBlock + "async function cmdCredits(");

  const modelsBlock = buildTarget(String.raw`async function cmdModels(chatId, userId) {
  const plan = await getPlan(userId);
  const text = __BT__✨ PIXLEMORPHIC MODEL CATALOG\n\n🟢 LIVE NOW\n🎬 Pixlemeta Cinematic\n📸 Pixlemeta Realism\n🟪 Pixlemeta Ultra 8K\n✏️ Pixlemeta EDIT — FLUX.1 Kontext Pro\n\n🟡 COMING SOON\n🍌 Nano Banana 2\n🧠 GPT Image 2\n🌱 Seedream 4.0\n⚡ FLUX.1 [dev]\n\n🎬 VIDEO\n⚡ Seedance 2.0 Fast\n🎞️ Seedance 2.0\n🔥 Seedance 2.5\n🎥 Kling 2.5 Turbo Pro\n🌊 WAN 2.2\n🟢 Veo 3.1\n\nPlan: ${plan.toUpperCase()}__BT__;
  return sendMessage(chatId, text, { reply_markup: homeKeyboard() });
}

`);
  src = replaceBetween(src, "async function cmdModels(", "/* =========================\n   GENERATION\n========================= */", modelsBlock + "/* =========================\n   GENERATION\n========================= */");

  const callbackMarker = "\n  if (\n    data.startsWith(\"m:\")";
  const callbackInsert = buildTarget(String.raw`
  if (data.startsWith("soon:")) {
    return sendMessage(chatId,
      __BT__🟡 COMING SOON\n\nThis model is listed in the new UI, but its provider integration is scheduled for the next integration pass. No credits were charged.__BT__,
      { reply_markup: imageKeyboard() }
    );
  }
  ...
  if (data === "v:soon") {
    return sendMessage(chatId,
      __BT__🟡 VIDEO MODEL — COMING SOON\n\nThe model is visible now, but generation is intentionally disabled until provider integration and cost testing are complete. No credits were charged.__BT__,
      { reply_markup: videoKeyboard() }
    );
  }
  ...
  if (data === "v:quality") {
    return sendMessage(chatId,
      __BT__🎚️ VIDEO QUALITY\n\n480p • Economy\n580p • Economy+\n720p • Standard\n1080p • High\n4K • Premium\n\nFinal availability will be enforced per model.__BT__,
      { reply_markup: videoKeyboard() }
    );
  }
  ...
  if (data === "v:duration") {
    return sendMessage(chatId,
      __BT__⏱️ VIDEO DURATION\n\n5s • Trial-safe\n8s • Standard\n10s • Cinematic\n15s • Premium\n\nFinal limits will be enforced per model.__BT__,
      { reply_markup: videoKeyboard() }
    );
  }
  ...
  if (data === "v:fps") {
    return sendMessage(chatId,
      __BT__🎞️ FRAME RATE\n\n24 FPS • Cinematic standard\n30 FPS • Smooth motion\n\nSome providers support additional FPS values. The backend will validate the selected model when activated.__BT__,
      { reply_markup: videoKeyboard() }
    );
  }
  ...
  if (data === "v:audio") {
    return sendMessage(chatId,
      __BT__🔊 AUDIO\n\nON • Native audio where supported\nOFF • Silent generation\n\nAudio support and pricing are provider/model specific.__BT__,
      { reply_markup: videoKeyboard() }
    );
  }
  ...
  if (data.startsWith("v:ratio:")) {
    ...
    return sendMessage(chatId,
      __BT__📐 FRAME RATIO\n\nSelected: ${ratio}\n\nPreview selection only. Provider-specific validation will run when video generation is enabled.__BT__,
      { reply_markup: videoKeyboard() }
    );
  }
`);
  if (src.indexOf(callbackMarker) < 0) throw new Error("Callback insertion marker missing");
...
try {
  ...
  console.log("Bootstrap: Pixlemeta EDIT + upgraded Image/Video UI activated");
  require(indexPath);
} catch (error) {
